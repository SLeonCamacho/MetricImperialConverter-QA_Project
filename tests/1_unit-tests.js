...
suite("Unit Tests", function () {
  ...
  test("Test error return on double fraction (i.e. 3/2/3)", () => {
    assert.equal(
      ...
      undefined,
      "convertHandler shoud correctly return an error on a double-fraction."
    );
  });
  test("Test default value when no number is given", () => {
    assert.equal(
      ...
      1,
      "convertHandler should correctly return a numerical input of 1 when no numerical input is provided."
    );
  });
  test("Test reading each valid input unit", () => {
    assert.equal(
      ...
      "gal",
      "convertHandler should return correct initUnit"
    );
    assert.equal(
  ...
  test("Test if an error is returned for an invalid unit", () => {
    assert.equal(
      convertHandler.getUnit("m"),
      undefined,
      "convertHandler should correctly return an error if an invalid input unit is entered."
    );
  });
  test("Test the return unit of each valid input unit", () => {
    assert.equal(
      ...
      "L",
      "convertHandler should return L for gal"
    );
    assert.equal(
      convertHandler.getReturnUnit("L"),
      "gal",
      "convertHandler should return gal for L"
    );
    assert.equal(
      convertHandler.getReturnUnit("mi"),
      "km",
      "convertHandler should return km for mi"
    );
    assert.equal(
      convertHandler.getReturnUnit("km"),
      "mi",
      "convertHandler should return mi for km"
    );
    assert.equal(
      convertHandler.getReturnUnit("lbs"),
      "kg",
      "convertHandler should return kg for lbs"
    );
    assert.equal(
      convertHandler.getReturnUnit("kg"),
      "lbs",
      "convertHandler should return lbs for kg"
    );
  });
  test("Test the spelled-out string return", () => {
    assert.equal(
      ...
      "gallons",
      "gal should spell out as gallons"
    );
    assert.equal(
      convertHandler.spellOutUnit("L"),
      "liters",
      "L should spell out as liters"
    );
    assert.equal(
      convertHandler.spellOutUnit("mi"),
      "miles",
      "mi should spell out as miles"
    );
    assert.equal(
      convertHandler.spellOutUnit("km"),
      "kilometers",
      "km should spell out as kilometers"
    );
    assert.equal(
      convertHandler.spellOutUnit("lbs"),
      "pounds",
      "lbs should spell out as pounds"
    );
    assert.equal(
      convertHandler.spellOutUnit("kg"),
      "kilograms",
      "kg should spell out as kilograms"
    );
  });
  test("Test the conversion of gal to L", () => {
    assert.equal(
      ...
      3.78541,
      "convertHandler should correctly covert gal to L"
    );
  });
  test("Test the conversion of L to gal", () => {
    assert.equal(
      ...
      0.26417,
      "convertHandler should correctly covert L to gal"
    );
  });
  test("Test the conversion of mi to km", () => {
    assert.equal(
      ...
      1.60934,
      "convertHandler should correctly covert mi to km"
    );
  });
  test("Test the conversion of km to mi", () => {
    assert.equal(
      ...
      0.62137,
      "convertHandler should correctly covert km to mi"
    );
  });
  test("Test the conversion of lbs to kg", () => {
    assert.equal(
      ...
      0.45359,
      "convertHandler should correctly covert lbs to kg"
    );
  });
  test("Test the conversion of kg to lbs", () => {
    assert.equal(
      ...
      2.20462,
      "convertHandler should correctly covert kg to lbs"
    );
  });
});